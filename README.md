...
GitHub Actionsで30分おきにフライト状態を監視し、出発・到着・大幅遅延・欠航時にLINEへ通知します。
...
5分間隔で監視すると長距離便1便だけで100リクエストを超える可能性があります。頻繁に使う場合は AviationStack の有料プラン、または監視頻度のさらなる調整を検討してください。
...
- **API使用量を節約したい**: ワークフローの cron を `0 * * * *`(1時間間隔)に変更
...
- 30分間隔のcronは GitHub Actions の負荷状況により数分ずれることがあります
- 結果として通知の遅延は **30〜40分程度** が目安です
